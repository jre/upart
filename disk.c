...
up_disk_setup(struct disk *disk, const struct disk_params *params)
{
	if (UP_DISK_IS_IMG(disk))
		...
		up_img_getparams(disk->img, &disk->params);
	else if (!UP_DISK_IS_FILE(disk))
		/* try to get drive parameters via OS-dependent interfaces */
		if (up_os_getparams(disk->fd, &disk->params,
		    UP_DISK_PATH(disk)) < 0)
			return (-1);
...
int64_t
up_disk_read(const struct disk *disk, int64_t start, int64_t size,
    void *buf, size_t bufsize)
{
    ...
    if(UP_DISK_IS_IMG(disk))
        return up_img_read(disk->img, start, size, buf);

    /* otherwise try to read from the disk */
    res = pread(disk->fd, buf, size * UP_DISK_1SECT(disk),
