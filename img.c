...
static uint32_t	img_checkcrc(struct up_imghdr_p *, int, const char *);
...
int
up_img_load(int fd, const char *name, struct up_img **ret)
{
    struct up_imghdr_p  hdr;
    void               *data;
    ssize_t             res;
    ...
    *ret = NULL;
    ...
    if(IMG_MINOR < UP_BETOH16(hdr.minor) && UP_NOISY(QUIET))
